...
test_should_clear_oclweb_releases_when_files_older_than_one_month() {
  ...
  TMP_TEST_DIR="/tmp/test_oclweb"

  mkdir -p $TMP_TEST_DIR/
  rm -f $TMP_TEST_DIR/*

  touch -a -m -t $SEVEN_DAYS_AGO_FORMAT $TMP_TEST_DIR/ocl_web$SEVEN_DAYS_AGO_FILE_NAME.tgz
  touch -a -m -t $SEVEN_DAYS_AGO_FORMAT $TMP_TEST_DIR/otherfile$SEVEN_DAYS_AGO_FILE_NAME.tgz
  touch -a -m -t $FORTY_DAYS_AGO_FORMAT $TMP_TEST_DIR/ocl_web$FORTY_DAYS_AGO_FILE_NAME.tgz
  ...
  assert_equals "3" $(find $TMP_TEST_DIR/ -type f | wc -l) "Should clear older than 30 days ocl_web files"
